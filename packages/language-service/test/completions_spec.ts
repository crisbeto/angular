...
describe('completions', () => {
  ...
  describe('let declarations', () => {
    ...
    it('should complete a chain of let declarations without a terminating character', () => {
      const {templateFile} = setup('@let foo = 1, bar = ', `title!: string; hero!52: number;`);
      templateFile.moveCursorToText('bar = ¦');
      const completions = templateFile.getCompletionsAtPosition();
      expectContain(completions, ts.ScriptElementKind.memberVariableElement, ['title', 'hero']);
      expectContain(completions, DisplayInfoKind.LET, ['foo']);
    });
