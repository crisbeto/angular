  ...
  describe('query()', () => {
      ...
      @Component({
        ...
        template: `
            <div [@myAnimation]="exp" #parent>
              @for (item of items; track item) {<div class="item e-{{ item }}">
              </div>}
            </div>
          `,
        animations: [
    ...
    it('should properly apply pre styling before a stagger is issued', () => {
      @Component({
        ...
        template: `
          <div [@myAnimation]="exp">
            @for (item of items; track item) {<div class="item">
              {{ item }}
            </div>}
          </div>
        `,
    ...
    it('should find newly inserted items in the component via :enter', () => {
      @Component({
        ...
        template: `
            <div @myAnimation>
              @for (item of items; track item) {<div class="child">
                {{ item }}
              </div>}
            </div>
          `,
         ...
         @Component({
           ...
           template: `
            <div [@myAnimation]="items.length" class="parent" #container>
              ...
              @if (items.length == 0) {<div class="child">Leave!</div>}
            </div>
          `,
           animations: [
             trigger(
          ...
          template: `
            <div [@myAnimation]="items.length" class="parent">
              @for (item of items; track item) {
                <section>
   @if (item % 2 == 0) {    "item % 2 == 0">even {}{ item }}</div>
   @if (item % 2 == 1) {    "item % 2 == 1">odd {}{ item }}</div>
                </section>
              }
            </div>
          `,
         ...
         @Component({
           ...
           template: `
            <div [@myAnimation]="exp1" class="ancestor" #ancestor>
              @if (exp2) {<div class="parent" #parent>
                <div class="child"></div>
                <div class="child"></div>
              </div>}
            </div>
         ...
         @Component({
           ...
           template: `
            <div [@one]="exp1" [@two]="exp2" class="ancestor" #ancestor>
              <header>hello</header>
              @if (parentExp) {<div class="parent" #parent>
  ...
  describe('sub triggers', () => {
       ...
       () => {
         @Component({
            ...
            <div #parent class="parent" [@parent]="exp">
              @for (item of items; track item) {<div class="item" @child></div>}
            </div>
          `,
             animations: [
               trigger('parent', [transition(
    ...
    it('should emulate a leave animation on the nearest sub host elements when a parent is removed',
       fakeAsync(() => {
         @Component({
           selector: 'ani-cmp',
           template: `
            @if (exp) {<div @parent class="parent1" #parent>
    ...
    it('should emulate a leave animation on a sub component\'s inner elements when a parent leave animation occurs with animateChild',
       () => {
         @Component({
           selector: 'ani-cmp',
           template: `
            @if (exp) {<div @myAnimation class="parent">
              <child-cmp></child-cmp>
            </div>}
          `,
           animations: [
    ...
    it('should not cause a removal of inner @trigger DOM nodes when a parent animation occurs',
       fakeAsync(() => {
         @Component({
           selector: 'ani-cmp',
           template: `
            @if (exp) {<div @parent class="parent">
      ...
      @Component({
        ...
        template: `
            <div [@pageAnimation]="status">
              <header>
                @if (!loading) {<div class="title">{{ title }}</div>}
                @if (loading) {<div class="loading">loading...</div>}
              </header>
              <section>
                <div class="page">
                  @if (page1) {<div class="page1">
                    <@if (true) {div page 1</div>
}                  </div>}
                  @if (page2) {<div class="page2">
                    <@if (true) {div page 2</div>
}                  </div>}
                </div>
              </section>
            </div>
    ...
    it('should emulate leave animation callbacks for all sub elements that have leave triggers within the component',
       fakeAsync(() => {
         @Component({
           ...
           template: `
            @if (exp) {<div data-name="p" class="parent" @parent (@parent.start)="callback($event)" (@parent.done)="callback($event)">
    ...
    it('should scope :enter queries between sub animations', () => {
      @Component({
        ...
        template: `
               @if (exp1) {<div @parent class="container">
                 @if (exp2) {<div>
                   <div @child>
                     @if (exp3) {<div>
                       <div class="item"></div>
         ...
         @Component({
           ...
           template: `
            <div class="grand-parent" [@grandParentAnimation]="childPresent">
              ...
              @if (childPresent) {<div class="child"></div>}
              </div>
            </div>
          `,
           animations: [
