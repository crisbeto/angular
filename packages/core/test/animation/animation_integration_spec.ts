  ...
  describe('animation triggers', () => {
    it('should trigger a state change animation from void => state', () => {
      @Component({
        selector: 'if-cmp',
        template: `
          @if (exp) {<div [@myAnimation]="exp"></div>}
        `,
        animations: [trigger(
            'myAnimation',
            [transition(
    ...
    it('should only turn a view removal as into `void` state transition', () => {
      @Component({
        selector: 'if-cmp',
        template: `
          @if (exp1) {<div [@myAnimation]="exp2"></div>}
        `,
        animations: [trigger(
            'myAnimation',
            [
      ...
      it('should trigger a leave animation when the outer components element binding updates on the host component element',
         fakeAsync(() => {
           @Component({
             ...
             template: `
                @if (exp) {<child-cmp @host></child-cmp>}
              `
           })
           class ParentCmp {
             public exp = true;
      ...
      it('should properly evaluate pre/auto-style values when components are inserted/removed which contain host animations',
         fakeAsync(() => {
           @Component({
             selector: 'parent-cmp',
             template: `
                @for (item of items; track item) {<child-cmp></child-cmp>}
              `
           })
           class ParentCmp {
             items: any[] = [1, 2, 3, 4, 5];
    ...
    it('should invoke an animation trigger that is state-less', () => {
      @Component({
        selector: 'ani-cmp',
        template: `
            @for (item of items; track item) {<div @myAnimation></div>}
          `,
        animations: [trigger(
            'myAnimation',
    ...
    it('should retain state styles when the underlying DOM structure changes even if there are no insert/remove animations',
       () => {
         @Component({
           selector: 'ani-cmp',
           template: `
            @for (item of items; track item) {<div class="item" [@color]="colorExp">
              {{ item }}
            </div>}
          `,
    ...
    it('should keep/restore the trigger value when there are move operations (with *ngFor + trackBy)',
       fakeAsync(() => {
         @Component({
           selector: 'ani-cmp',
           template: `
          @for (item of items, trackBy: trackItem; track item) {<div
    ...
    it('should animate removals of nodes to the `void` state for each animation trigger, but treat all auto styles as pre styles',
       () => {
         @Component({
           selector: 'ani-cmp',
           template: `
            @if (exp) {<div class="ng-if" [@trig1]="exp2" @trig2></div>}
          `,
           animations: [
         ...
         @Component({
           ...
           template: `
            <div #parent [@parent]="exp1" class="parent">
              @if (exp2) {<div #child class="child"></div>}
            </div>
          `,
           animations: [trigger(
               'parent',
    ...
    it('should make inner removals wait until a parent based removal animation has finished',
       () => {
         @Component({
           selector: 'ani-cmp',
           template: `
            @if (exp1) {<div #parent @parent class="parent">
              @if (exp2) {<div #child1 class="child1"></div>}
              @if (exp2) {<div #child2 class="child2"></div>}
            </div>}
          `,
           animations: [trigger(
               'parent',
    ...
    it('should animate nodes properly when they have been re-ordered', () => {
      @Component({
        selector: 'if-cmp',
        template: `
                @for (item of items; track item) {<div [class]="'class-' + item.value">
    ...
    it('should trigger a `start` state change listener for when the animation changes state from void => state',
       fakeAsync(() => {
         @Component({
           selector: 'if-cmp',
           template: `
          @if (exp) {<div [@myAnimation]="exp" (@myAnimation.start)="callback($event)"></div>}
        `,
           animations: [trigger(
               'myAnimation',
               [transition(
    ...
    it('should trigger a `done` state change listener for when the animation changes state from a => b',
       fakeAsync(() => {
         @Component({
           selector: 'if-cmp',
           template: `
          @if (exp) {<div [@myAnimation123]="exp" (@myAnimation123.done)="callback($event)"></div>}
        `,
           animations: [trigger(
               'myAnimation123',
               [transition(
    ...
    it('should fire callback events for leave animations even if there is no leave transition',
       fakeAsync(() => {
         @Component({
           selector: 'my-cmp',
           template: `
              @if (exp) {<div @myAnimation (@myAnimation.start)="callback($event)" (@myAnimation.done)="callback($event)"></div>}
            `,
           animations: [trigger('myAnimation', [])]
         })
         class Cmp {
             ...
             template: `
              <div [@.disabled]="disableExp" #container>
                <div [@parent]="exp" (@parent.done)="onDone($event)">
                  @for (item of items; track item) {<div class="item" @child (@child.done)="onDone($event)"></div>}
                </div>
              </div>
                `
           })
