    ...
    @Component({
      ...
      template: `
            <div [@myAnimation]="exp" #parent>
              @for (item of items; track item) {<div class="child" style="line-height:20px">
                - {{ item }}
              </div>}
            </div>
          `,
       ...
       @Component({
          ...
          template: `
              ...
              <div [@list]="exp" class="list">
                @for (item of items; track item) {<div class="outer">
                  <div class="inner">
                    {{ item }}
                  </div>
                </div>}
  ...
  it('should compute intermediate styles properly for multiple queried elements when an animation is cancelled',
     () => {
       @Component({
         ...
         template: `
          <div [@myAnimation]="exp">
            @for (item of items; track item) {<div class="target"></div>}
          </div>
        `,
         animations: [
           trigger(
