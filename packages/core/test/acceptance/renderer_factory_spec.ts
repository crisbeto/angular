...
describe('Renderer2 destruction hooks', () => {
  @Component({
    selector: 'some-component',
    template: `
      @if (isContentVisible) {<span>A</span>}
      @if (isContentVisible) {<span>B</span>}
      @if (isContentVisible) {<span>C</span>}
    `,
  })
  class SimpleApp {
  ...
  @Component({
    selector: 'some-component',
    template: `
      @if (isContentVisible) {<basic-comp>A</basic-comp>}
      @if (isContentVisible) {<basic-comp>B</basic-comp>}
      @if (isContentVisible) {<basic-comp>C</basic-comp>}
    `,
  })
