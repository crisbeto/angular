...
describe('providers', () => {
  ...
  describe('lifecycles', () => {
    ...
    it('should not invoke ngOnDestroy on multi providers', () => {
      const destroyCalls: (DestroyService | OtherDestroyService)[] = [];
      const SERVICES = new InjectionToken<any>('SERVICES');
      let destroyServiceInstance: DestroyService|undefined;
      let otherDestroyServiceInstance: DestroyService|undefined;

      @Injectable()
      class DestroyService {
        constructor() { destroyServiceInstance = this; }
        ngOnDestroy() { destroyCalls.push(this); }
      }

      @Injectable()
      class OtherDestroyService {
        constructor() { otherDestroyServiceInstance = this; }
        ngOnDestroy() { destroyCalls.push(this); }
      }
      ...
      class App {
        constructor(@Inject(SERVICES) services: [DestroyService, OtherDestroyService]) {}
      }
      ...
      expect(destroyServiceInstance).toBeTruthy();
      expect(otherDestroyServiceInstance).toBeTruthy();
      expect(destroyCalls).toEqual([destroyServiceInstance !, otherDestroyServiceInstance !]);
    });
