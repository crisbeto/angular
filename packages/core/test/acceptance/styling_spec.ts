...
describe('styling', () => {
      ...
      @Component({
        ...
        template: `
          <div class="my-class" [ngClass]="classMap" test-dir>
            @if (showing) {<div child-dir>Hello</div>}
          </div>
        `
      })
      class AppComponent {
     ...
     () => {
       @Component({
         template: `
            ...
            @for (item of items; track item) {<div class="item item-{{ item }}">
              {{ item }}
            </div>}
  ...
  it('should combine static host classes with component "class" host attribute', () => {
    ...
    @Component({
      template: `@for (item of items; track item) {<comp-with-classes class="inline"></comp-with-classes>}`
    })
    class MyComp {
      items = [1, 2, 3];
    }
