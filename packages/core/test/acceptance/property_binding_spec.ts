...
describe('property bindings', () => {
    ...
    it('should support unrelated element properties at same index in if-else block', () => {
      @Component({
        template: `
          ...
          @if (condition) {<button [id]="id2">Click me too (2)</button>}
          @if (!condition) {<button otherDir [id]="id3">Click me too (3)</button>}
        `
      })
      class App {
        condition = true;
  ...
  describe('attributes and input properties', () => {
    ...
    it('should support attributes at same index inside an if-else block', () => {
      @Component({
        template: `
          ...
          @if (condition) {<div role="button" myDirB></div>}
          @if (!condition) {<div role="menu"></div>}
        `,
      })
      class App {
        condition = true;
    ...
    it('should process attributes properly inside a for loop', () => {
      ...
      @Component({
        template: `
          @for (i of [0, 1]; track i) {<comp></comp>}
        `
      })
      class App {
      }
