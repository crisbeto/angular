  ...
  describe('NgForOf', () => {
    ...
    it('should support multiple levels of embedded templates', () => {
      @Component({
        selector: 'app-multi',
        template: `<ul>
          @for (row of items; track row) {<li>
            @for (cell of row.data; track cell) {<span>{{cell}} - {{ row.value }} - {{ items.length }}</span>}
          </li>}
       </ul>`
      })
    ...
    it('should support multiple levels of embedded templates with listeners', () => {
      @Component({
        selector: 'app-multi',
        template: `@for (row of items; track row) {<div>
          @for (cell of row.data; track cell) {<p>
    ...
    it('should support skipping contexts', () => {
      @Component({
        selector: 'app-multi',
        template: `@for (row of items; track row) {<div>
           @for (cell of row; track cell) {<div>
              @for (span of cell.data; track span) {<span>{{ cell.value }} - {{ name }}</span>}
           </div>}
        </div>}`
      })
    ...
    it('should support context for 9+ levels of embedded templates', () => {
      @Component({
        selector: 'app-multi',
        template: `@for (item0 of items; track item0) {<div>
            @for (item1 of item0.data; track item1) {<span>
               @for (item2 of item1.data; track item2) {<span>
                   @for (item3 of item2.data; track item3) {<span>
                       @for (item4 of item3.data; track item4) {<span>
                           @for (item5 of item4.data; track item5) {<span>
                               @for (item6 of item5.data; track item6) {<span>
                                   @for (item7 of item6.data; track item7) {<span>
                                       @for (item8 of item7.data; track item8) {<span>{{ item8 }}.{{ item7.value }}.{{ item6.value }}.{{ item5.value }}.{{ item4.value }}.{{ item3.value }}.{{ item2.value }}.{{ item1.value }}.{{ item0.value }}.{{ value }}</span>}
                                   </span>}
                               </span>}
                           </span>}
                       </span>}
  ...
  describe('ngIf', () => {
    it('should support sibling ngIfs', () => {
      @Component({
        selector: 'app-multi',
        template: `
          @if (showing) {<div>{{ valueOne }}</div>}
          @if (showing) {<div>{{ valueTwo }}</div>}
        `
      })
    ...
    it('should handle nested ngIfs with no intermediate context vars', () => {
      @Component({
        selector: 'app-multi',
        template: `@if (showing) {<div>
          @if (outerShowing) {<div>
              @if (innerShowing) {<div>{{ name }}</div>}
            </div>}
          </div>}
        `
      })
