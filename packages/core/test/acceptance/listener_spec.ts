  ...
  describe('destroy', () => {
    it('should destroy listeners when view is removed', () => {
      @Component({
        selector: 'my-comp',
        template: `
          @if (visible) {<button (click)="count()">Click me!</button>}
        `,
      })
      class MyComp {
        visible = true;
    ...
    it('should destroy listeners when views generated using *ngFor are removed', () => {
      ...
      @Component({
        selector: 'my-comp',
        template: `
          @for (button of buttons; track button) {<button (click)="count()">Click me!</button>}
        `,
      })
      class MyComp {
        buttons = [1, 2];
    ...
    it('should destroy listeners when nested view is removed', () => {
      @Component({
        selector: 'my-comp',
        template: `
          @if (isSectionVisible) {<ng-container>
            Click to submit a form:
            @if (isButtonVisible) {<button (click)="count()">Click me!</button>}
          </ng-container>}
        `,
      })
      class MyComp {
