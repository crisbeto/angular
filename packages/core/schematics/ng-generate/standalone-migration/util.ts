...
export class ChangeTracker {
   ...
   */
  replaceText(sourceFile: ts.SourceFile, start: number, length: number, text: string): void {
    this._trackChange(sourceFile, {start, length, text});
  }
   ...
   * whent copying nodes from one file to another, because TypeScript might not output literal nodes
   * without it.
   */
  replaceNode(
  ...
  removeNode(node: ts.Node): void {
    this._trackChange(
        node.getSourceFile(), {start: node.getStart(), length: node.getWidth(), text: ''});
  }
