...
import {ChangeTracker, createLanguageService, findClassDeclaration, findLiteralProperty, getNodeLookup, getRelativeImportPath, NodeLookup, offsetsToNodes} from './util';
...
interface BootstrapCallAnalysis {
  ...
  component: ts.ClassDeclaration&{name: ts.Identifier};
}
...
export function toStandaloneBootstrap(
  ...
  sourceFiles.forEach(function walk(node: ts.Node) {
    if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression) &&
        ...
        isClassReference(node.expression, 'PlatformRef', '@angular/core', typeChecker)) {
...
function analyzeBootstrapCall(
  ...
  if (component && component.name && ts.isIdentifier(component.name)) {
    return {
      module: declaration,
      metadata,
      component: component as ts.ClassDeclaration & {name: ts.Identifier},
      call
    };
  }
...
function migrateBootstrapCall(
  ...
  replaceBootstrapCall(analysis, providersInNewCall, moduleImportsInNewCall, tracker);
}
...
function replaceBootstrapCall(
  ...
  tracker.replaceNode(
      ...
      // Note: it's important to pass in the metadata source file! Otherwise TS won't print out
      // literals inside of the providers that we're copying over from the module file.
 ...
 * @param typeChecker
 */
function migrateImportsInBootstrapCall(
  ...
  for (const element of imports.initializer.elements) {
    ...
    if (ts.isIdentifier(element)) {
      ...
      if (isClassReference(element, 'BrowserAnimationsModule', animationsModule, typeChecker)) {
        providersInNewCall.push(ts.factory.createCallExpression(
      ...
      if (isClassReference(element, 'NoopAnimationsModule', animationsModule, typeChecker)) {
        providersInNewCall.push(ts.factory.createCallExpression(
...
function referencesToNodeWithinSameFile(
  ...
  if (highlights) {
    ...
    if (offsets.length > 0) {
      const nodes = offsetsToNodes(nodeLookup, offsets);
