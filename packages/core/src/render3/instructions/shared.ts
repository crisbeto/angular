...
function generateInitialInputs(
  ...
  while (i < attrs.length) {
    ...
    if (inputs.hasOwnProperty(attrName as string)) {
      const privateName = inputs[attrName as string];

      if (hostDirectiveInputs === null) {
        if (inputsToStore === null) inputsToStore = [];
        inputsToStore.push(attrName as string, privateName, attrs[i + 1] as string);
      } else if (
          hostDirectiveInputs.hasOwnProperty(privateName) &&
          hostDirectiveInputs[privateName] === attrName) {
        if (inputsToStore === null) inputsToStore = [];
        inputsToStore.push(attrName as string, privateName, attrs[i + 1] as string);
      }
    }
