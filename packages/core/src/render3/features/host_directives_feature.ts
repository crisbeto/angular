...
function applyHostDirectives(
  ...
  if (def.hostDirectives !== null) {
    ...
    for (let i = def.hostDirectives.length - 1; i > -1; i--) {
      ...
      hostDirectiveResults.set(hostDirectiveDef, hostDirectiveConfig);
...
function patchDeclaredInputs(
    declaredInputs: {[key: string]: string}, exposedInputs: HostDirectiveMapping): void {
...
function validateHostDirective(
    reference: Type<unknown>, def: DirectiveDef<any>|null, exposedInputs: HostDirectiveMapping,
    exposedOutputs: HostDirectiveMapping, matches: any[]): asserts def is DirectiveDef<unknown> {
  if (def === null) {
...
function validateMappings(
    ...
    hostDirectiveMappings: HostDirectiveMapping) {
