...
export function insertView(lView: LView, lContainer: LContainer, index: number) {
  ...
  const views = lContainer.slice(CONTAINER_HEADER_OFFSET);
  if (index > 0) {
  ...
  if (index < views.length) {
    ...
    lContainer.splice(index + CONTAINER_HEADER_OFFSET, 0, lView);
  } else {
    if (lContainer.length < CONTAINER_HEADER_OFFSET) {
      lContainer[CONTAINER_HEADER_OFFSET] = lView;
    } else {
...
function appendProjectedNode(
  ...
  if (projectedTNode.type === TNodeType.Container) {
    ...
    const views = nodeOrContainer.slice(CONTAINER_HEADER_OFFSET);
