...
function createNodeFromListenerDecorator(
  ...
  const eventNameNode = decorator.eventName || decorator.memberName;
...
function fixupSpans(ast: AST, node: StaticSourceNode): void {
  ...
  if (escapeIndex > -1) {
    // -1/+1 so that we capture the quotes of the initializer.
    const start = node.sourceSpan.start.offset - 1;
    const end = node.sourceSpan.end.offset + 1;
