...
export function generateTypeCtorDeclarationFn(
  ...
  const typeRef = ts.isIdentifier(nodeTypeRef)
    ? nodeTypeRef.text
    : TcbNode.tempPrint(nodeTypeRef, nodeTypeRef.getSourceFile());
...
function constructTypeCtorParameter(
  ...
  for (const {classPropertyName, transform, isSignal} of meta.fields.inputs) {
    if (isSignal) {
      ...
      const coercionType =
        transform != null
          ? TcbNode.tempPrint(transform.type.node, sourceFile)
  ...
  if (signalInputKeys.length > 0) {
    ...
    const unwrapRef = env.tcbReferenceExternalSymbol(
...
function typeParametersWithDefaultTypes(
  ...
  const paramStrings = params.map((param) => {
    const constraint = param.constraint
      ? ` extends ${TcbNode.tempPrint(param.constraint, param.getSourceFile())}`
      : '';
    const defaultValue = ` = ${
      param.default ? TcbNode.tempPrint(param.default, param.getSourceFile()) : 'any'
    }`;
