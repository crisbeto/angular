...
export function generateTypeCheckBlock(
  ...
  const typeParamsStr =
    typeParameters === undefined || typeParameters.length === 0
      ? ''
      : `<${typeParameters.map((p) => TcbNode.tempPrint(p, sourceFile)).join(', ')}>`;
  const typeArgsStr =
    typeArguments === undefined || typeArguments.length === 0
      ? ''
      : `<${typeArguments.map((p) => TcbNode.tempPrint(p, sourceFile)).join(', ')}>`;
  const typeRef = ts.isIdentifier(ctxRawType.typeName)
    ? ctxRawType.typeName.text
    : TcbNode.tempPrint(ctxRawType.typeName, sourceFile);
...
function renderBlockStatements(env: Environment, scope: Scope, wrapperExpression: string): string {
  // Note: this needs to be called so that it can populate the prelude statements.
  const scopeStatements = scope.render();
  const statements = [...env.getPreludeStatements(), ...scopeStatements].map((stmt) =>
    stmt.printAsStatement(),
  );
