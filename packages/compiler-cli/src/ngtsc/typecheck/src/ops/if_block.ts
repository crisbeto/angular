...
export class TcbIfOp extends TcbOp {
  ...
  private generateBranch(index: number): TcbNode | undefined {
    ...
    if (branch.expression === null) {
      ...
      return new TcbNode(TcbNode.printAsBlock(branchScope.render()));
    }
    ...
    const ifStatement = `if (${expression.print()}) ${TcbNode.printAsBlock(bodyScope.render())}`;
    const elseBranch = this.generateBranch(index + 1);

    return new TcbNode(ifStatement + (elseBranch ? ' else ' + elseBranch.print() : ''));
  }
  ...
  private generateBranchGuard(index: number): TcbNode | null {
    ...
    for (let i = 0; i <= index; i++) {
      ...
      const comparisonExpression =
        i === index ? expression : new TcbNode(`!(${expression.print()})`);

      // Finally add the expression to the guard with an && operator.
      guard =
        guard === null
          ? comparisonExpression
          : new TcbNode(`${guard.print()} && ${comparisonExpression.print()}`);
    }
