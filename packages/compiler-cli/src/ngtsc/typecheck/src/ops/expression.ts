...
import {astToTcbNode} from '../expression';
 ...
 */
export function tcbExpression(ast: AST, tcb: Context, scope: Scope): TcbNode {
 ...
 */
export function unwrapWritableSignal(expression: TcbNode, tcb: Context): TcbNode {
  const unwrapRef = tcb.env.tcbReferenceExternalSymbol(
    R3Identifiers.unwrapWritableSignal.moduleName,
    R3Identifiers.unwrapWritableSignal.name,
  );
  return new TcbNode(`${unwrapRef.print()}(${expression.print()})`);
}
...
export class TcbConditionOp extends TcbOp {
  ...
  override execute(): null {
    ...
    this.scope.addStatement(new TcbNode(`if (${expr.print()}) {}`));
    return null;
  }
}
...
export class TcbExpressionTranslator {
  ...
  translate(ast: AST): TcbNode {
    // `astToTcbNode` actually does the conversion. A special resolver `tcbResolve` is passed
    // which interprets specific expression nodes that interact with the `ImplicitReceiver`. These
    // nodes actually refer to identifiers within the current scope.
    return astToTcbNode(ast, (ast) => this.resolve(ast), this.tcb.env.config);
  }
   ...
   */
  protected resolve(ast: AST): TcbNode | null {
    if (ast instanceof PropertyRead && ast.receiver instanceof ImplicitReceiver) {
      ...
      if (
        ...
        if (targetExpression !== null) {
          return new TcbNode(`${targetExpression.print()} as any`);
        }
      }
      ...
      const result = new TcbNode(`(${targetExpression.print()} = ${expr.print()})`);
      ...
      let methodAccess = new TcbNode(`${pipe.print()}.transform`).addParseSpanInfo(ast.nameSpan);

      if (!this.tcb.env.config.checkTypeOfPipes) {
        methodAccess = new TcbNode(`(${methodAccess.print()} as any)`);
      }

      const result = new TcbNode(`${methodAccess.print()}(${[expr.print(), ...args].join(', ')})`);
      ...
      if (
        ...
        const result = new TcbNode(`(${expr.print()} as any)`);
      ...
      method.addParseSpanInfo(ast.receiver.nameSpan).wrapForDiagnostics();
      const args = ast.args.map((arg) => this.translate(arg).print());
      const node = new TcbNode(`${method.print()}(${args.join(', ')})`);
  ...
  private getTargetNodeExpression(targetNode: TemplateEntity, expressionNode: AST): TcbNode {
