...
export class TcbTemplateContextOp extends TcbOp {
  ...
  override execute(): TcbNode {
    ...
    const ctx = new TcbNode(this.tcb.allocateId());
    this.scope.addStatement(TcbNode.declareVariable(ctx, new TcbNode('any')));
    return ctx;
  }
}
  ...
  override execute(): null {
    ...
    if (directiveGuards.length > 0) {
      ...
      guard = directiveGuards.reduce(
        (expr, dirGuard) => new TcbNode(`${expr.print()} && ${dirGuard.print()}`),
        directiveGuards.pop()!,
      );
    }
    ...
    let tmplBlock = TcbNode.printAsBlock(statements);
    if (guard !== null) {
      // The scope has a guard that needs to be applied, so wrap the template block into an `if`
      // statement containing the guard expression.
      tmplBlock = `if (${guard.print()}) ${tmplBlock}`;
    }
    this.scope.addStatement(new TcbNode(tmplBlock));
