...
export class TcbSwitchOp extends TcbOp {
  ...
  override execute(): null {
    ...
    const clauses = this.block.groups.flatMap<TcbNode>((current) => {
      ...
      const statements = [...clauseScope.render(), new TcbNode('break')];

      return current.cases.map((switchCase, index) => {
        const statementsStr = (index === current.cases.length - 1 ? statements : [])
          .map((stmt) => stmt.printAsStatement())
          .join(' ');
    ...
    if (this.block.exhaustiveCheck) {
      ...
      clauses.push(
        new TcbNode(`default: const tcbExhaustive${exhaustiveId}: never = ${switchValue.print()};`),
      );
    }
  ...
  private generateGuard(group: TmplAstSwitchBlockCaseGroup, switchValue: TcbNode): TcbNode | null {
    ...
    if (!hasDefault) {
      ...
      for (const switchCase of group.cases) {
        if (switchCase.expression !== null) {
          ...
          const comparison = new TcbNode(`${switchValue.print()} === ${expression.print()}`);

          if (guard === null) {
            guard = comparison;
          } else {
            guard = new TcbNode(`${guard.print()} || ${comparison.print()}`);
          }
        }
      }
