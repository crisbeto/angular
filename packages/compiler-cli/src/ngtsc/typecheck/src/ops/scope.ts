...
export class Scope {
   ...
   *
   * This array can contain either a `TcbOp` which has yet to be executed, or a `TcbNode|null`
   ...
   * circularly.
   */
  private opQueue: (TcbOp | TcbNode | null)[] = [];
   ...
   */
  private varMap = new Map<TmplAstVariable, number | TcbNode>();
  ...
  private constructor(
    ...
    private guard: TcbNode | null = null,
  ) {}
  ...
  static forNodes(
    ...
    if (scopedNode instanceof TmplAstTemplate) {
      ...
      const loopInitializer = new TcbNode(tcb.allocateId()).addParseSpanInfo(
        scopedNode.item.sourceSpan,
      );
      ...
      for (const variable of scopedNode.contextVariables) {
        ...
        const type = new TcbNode(forLoopContextVariableTypes.get(variable.value)!);
        Scope.registerVariable(
          scope,
          variable,
   ...
   */
  resolve(node: LocalSymbol, directive?: TypeCheckableDirectiveMeta): TcbNode {
   ...
   */
  addStatement(stmt: TcbNode): void {
   ...
   */
  guards(): TcbNode | null {
    let parentGuards: TcbNode | null = null;
    if (this.parent !== null) {
  ...
  private resolveLocal(ref: LocalSymbol, directive?: TypeCheckableDirectiveMeta): TcbNode | null {
    if (ref instanceof TmplAstReference && this.referenceOpMap.has(ref)) {
      ...
      return typeof opIndexOrNode === 'number'
        ...
        : new TcbNode(opIndexOrNode.getSource());
    } else if (
  ...
  /**
   * Like `executeOp`, but assert that the operation actually returned `TcbNode`.
   */
  private resolveOp(opIndex: number): TcbNode {
   ...
   */
  private executeOp(opIndex: number, skipOptional: boolean): TcbNode | null {
    const op = this.opQueue[opIndex];
    if (!(op instanceof TcbOp)) {
      return op === null ? null : new TcbNode(op.getSource());
    }
