...
export class TcbNativeFieldOp extends TcbOp {
  ...
  override execute(): null {
    ...
    const expectedType = new TcbNode(this.getExpectedTypeFromDomNode(this.node));
    ...
    const assignment = new TcbNode(`${id.print()} = ${value.print()}`);
    assignment.addParseSpanInfo(fieldBinding.valueSpan ?? fieldBinding.sourceSpan);

    this.scope.addStatement(TcbNode.declareVariable(id, expectedType));
...
export class TcbNativeRadioButtonFieldOp extends TcbNativeFieldOp {
  ...
  override execute(): null {
    ...
    if (valueBinding !== undefined) {
      ...
      this.scope.addStatement(TcbNode.declareVariable(id, new TcbNode('string')));
      this.scope.addStatement(assignment);
    }
 ...
 */
function extractFieldValue(expression: AST, tcb: Context, scope: Scope): TcbNode {
  // Unwraps the field, e.g. `[field]="f"` turns into `f()`.
  const innerCall = new TcbNode(tcbExpression(expression, tcb, scope).print() + '()');
  ...
  return new TcbNode(`${innerCall.print()}.value()`);
}
