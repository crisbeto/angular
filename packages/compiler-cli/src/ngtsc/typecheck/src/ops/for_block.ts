...
export class TcbForOfOp extends TcbOp {
  ...
  override execute(): null {
    ...
    const initializer = new TcbNode(`const ${initializerId.print()}`).addParseSpanInfo(
      this.block.item.keySpan,
    );
    ...
    const expression = new TcbNode(
      `${tcbExpression(this.block.expression, this.tcb, this.scope).print()}!`,
    );
    const trackTranslator = new TcbForLoopTrackTranslator(this.tcb, loopScope, this.block);
    const trackExpression = trackTranslator.translate(this.block.trackBy);
    const block = TcbNode.printAsBlock([...loopScope.render(), trackExpression]);
    this.scope.addStatement(
      new TcbNode(`for (${initializer.print()} of ${expression.print()}) ${block}`),
    );
    return null;
  }
}
