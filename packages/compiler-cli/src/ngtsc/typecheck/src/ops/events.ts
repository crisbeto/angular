 ...
 * bindings.
 */
export function tcbEventHandlerExpression(ast: AST, tcb: Context, scope: Scope): TcbNode {
...
export class TcbDirectiveOutputsOp extends TcbOp {
  ...
  override execute(): null {
    ...
    for (const output of this.outputs) {
      ...
      const outputField = new TcbNode(`${dirId.print()}["${field}"]`).addParseSpanInfo(
        output.keySpan,
      );
      ...
      if (this.tcb.env.config.checkTypeOfOutputEvents) {
        ...
        const call = new TcbNode(
          `${outputField.print()}.subscribe(${handler.print()})`,
        ).addParseSpanInfo(output.sourceSpan);
    ...
    for (const output of this.outputs) {
      ...
      if (output.type === ParsedEventType.LegacyAnimation) {
        ...
        const eventType = this.tcb.env.config.checkTypeOfAnimationEvents
          ? this.tcb.env.tcbReferenceExternalSymbol('@angular/animations', 'AnimationEvent').print()
          : EventParamType.Any;

        const handler = tcbCreateEventHandler(output, this.tcb, this.scope, eventType);
        this.scope.addStatement(handler);
      } else if (output.type === ParsedEventType.Animation) {
        const eventType = this.tcb.env.tcbReferenceExternalSymbol(
          '@angular/core',
        ...
        // base `Event` type.
        let target: TcbNode;
        let domEventAssertion: TcbNode | undefined;
        ...
        if (output.target === 'window' || output.target === 'document') {
          target = new TcbNode(output.target);
        } else if (elId === null) {
        ...
        if (
          ...
          const assertUtil = this.tcb.env.tcbReferenceExternalSymbol(
            '@angular/core',
            'ɵassertType',
          );
          domEventAssertion = new TcbNode(
            `${assertUtil.print()}<typeof ${target.print()}>(${EVENT_PARAMETER}.target)`,
          );
        }

        const propertyAccess = new TcbNode(`${target.print()}.addEventListener`).addParseSpanInfo(
          output.keySpan,
        );
        ...
        const call = new TcbNode(`${propertyAccess.print()}("${output.name}", ${handler.print()})`);
...
class TcbEventHandlerTranslator extends TcbExpressionTranslator {
  protected override resolve(ast: AST): TcbNode | null {
    ...
    if (
      ...
      return new TcbNode(EVENT_PARAMETER).addParseSpanInfo(ast.nameSpan);
    }
...
function tcbCreateEventHandler(
  ...
  assertionExpression?: TcbNode,
): TcbNode {
  const handler = tcbEventHandlerExpression(event.handler, tcb, scope);
  const statements: TcbNode[] = [];
  ...
  if (event.type === ParsedEventType.TwoWay && tcb.env.config.checkTwoWayBoundEvents) {
    ...
    statements.push(
      new TcbNode(`var ${target} = ${initializer.print()}`),
      new TcbNode(`${target} = ${EVENT_PARAMETER}`),
    );
  } else {
  ...
  let body = TcbNode.printAsBlock(statements);
  ...
  return new TcbNode(`(${eventParam.print()}): any => ${body}`);
}
