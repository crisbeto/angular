      ...
      decorators.forEach((decorator) => {
        ...
        if (ts.isDecorator(decorator.node)) {
          ...
          if (member && ts.isMethodDeclaration(member) && member.name) {
            const memberName = sourceNodeFromTs(member.name) as StaticSourceNode;
