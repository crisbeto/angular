    ...
    DecoratorHandler<Decorator, ComponentAnalysisData, ComponentSymbol, ComponentResolutionData>
{
  ...
  private getSyntheticHostElement(
    ...
    for (const prop of hostBindings.properties) {
      ...
      // TODO: the `1` offset seems to be slightly off for `'[foo]': '123',`.
      const span = createSourceSpan(prop.initializer, 1);
      ...
      if (key.startsWith('(')) {
        ...
        inputs.push(
          new TmplAstBoundAttribute(
            ...
            type,
            0,
            parser.parseBinding(prop.initializer.text, true, span, prop.initializer.getStart() + 1),
            null,
            span,
            span,
            undefined,
