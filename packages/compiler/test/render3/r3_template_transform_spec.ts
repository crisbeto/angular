...
describe('R3 template transform', () => {
  ...
  describe('@let declarations', () => {
    ...
    it('should parse multiple let declarations in a single statement', () => {
      const template = '@let one = 1, two = 2, three = one + two;';
      expectLet(template).toEqual([
        ['LetDeclaration', 'one', '1'],
        ['LetDeclaration', 'two', '2'],
        ['LetDeclaration', 'three', 'one + two'],
      ]);
    });
    ...
    it('should produce a text node when @let is used inside ngNonBindable', () => {
      expectLet('<div ngNonBindable>@let foo = 123, bar = 456;</div>').toEqual([
        ['Element', 'div'],
        ['TextAttribute', 'ngNonBindable', ''],
        ['Text', '@let foo = 123;'],
        ['Text', '@let bar = 456;'],
