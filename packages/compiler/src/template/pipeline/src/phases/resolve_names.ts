...
function processLexicalScope(
  ...
  const scope = new Map<string, ir.VariableOp<ir.CreateOp | ir.UpdateOp>>();
  const letDecls = new Map<ir.XrefId, ir.StoreLetOp>();
      ...
      (expr) => {
        if (expr instanceof ir.LexicalReadExpr) {
          ...
          if (scope.has(expr.name)) {
            const varOp = scope.get(expr.name)!;
            const targetLetDecl =
              varOp.variable.kind === ir.SemanticVariableKind.LetReference &&
              letDecls.has(varOp.variable.target)
                ? letDecls.get(varOp.variable.target)!
                : null;
            ...
            if (targetLetDecl !== null && isIllegalForwardLetReference(op, varOp, targetLetDecl)) {
              return o.literal(undefined);
            }

            // This was a defined variable in the current scope.
            return new ir.ReadVariableExpr(varOp.xref);
          } else {
 ...
 * @param target Variable representing the resolve value of the `@let` declaration that
 *    the expression is targeting.
 * @param letDeclaration Op that declared the `@let`.
 */
function isIllegalForwardLetReference(
  expressionOp: ir.CreateOp | ir.UpdateOp,
  target: ir.VariableOp<ir.CreateOp | ir.UpdateOp>,
  letDeclaration: ir.StoreLetOp,
): boolean {
  if (
    expressionOp.kind !== ir.OpKind.Listener &&
    expressionOp.kind !== ir.OpKind.TwoWayListener &&
    target.variable.kind === ir.SemanticVariableKind.LetReference
  ) {
    let current = letDeclaration as ir.UpdateOp | null;
    while (current) {
