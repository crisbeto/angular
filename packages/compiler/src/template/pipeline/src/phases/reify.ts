  ...
  for (const op of ops) {
    ...
    switch (op.kind) {
      ...
      case ir.OpKind.Listener:
        ...
        ir.OpList.replace(
          op,
          unit.job.mode === CompilationMode.DomOnly && !op.hostListener && !op.isAnimationListener
  ...
  for (const op of ops) {
    ...
    switch (op.kind) {
      ...
      case ir.OpKind.Property:
        ir.OpList.replace(
          op,
          unit.job.mode === CompilationMode.DomOnly && !op.isAnimationTrigger
            ? ng.domProperty(op.name, op.expression as ir.Expression, op.sanitizer, op.sourceSpan)
