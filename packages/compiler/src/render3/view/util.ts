...
/** Checks if a key is safe to use directly in a read expression or it needs to be quoted. */
export function isUnsafeObjectKey(name: string): boolean {
  return UNSAFE_OBJECT_KEY_NAME_REGEXP.test(name);
}
